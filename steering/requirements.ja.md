...
お手本再生が開始されたとき、システムは録音中であれば録音を停止する。

#### REQ-RC-REC-007: 録音開始時のお手本再生停止

**種別**: Event-Driven

録音が開始されたとき、システムはお手本再生中であれば再生を停止する。
...
練習録音停止時、システムはスコアを算出してTakeとして保存し、インラインにスコアを表示する。ユーザーは「もう一度」で即次の録音を開始するか、「完了」でスコア結果画面へ遷移できる。
...
スコア結果画面において、システムはお手本と録音のピッチ差分を折れ線グラフ（横軸: 時間、縦軸: セント差 -100〜+100）で表示する。
...
ユーザーがスコア結果画面でundo（取消）ボタンをタップしたとき、システムは直前のTakeを削除して練習録音画面に戻る。

#### REQ-RC-DATA-007: フレーズ詳細からのTake削除

**種別**: Event-Driven

ユーザーがフレーズ詳細画面の練習履歴で削除ボタンをタップしたとき、システムは確認後にそのTakeを削除する。
...
| カテゴリ | 要件ID | 内容 |
|---------|--------|------|
| 再生排他 | PLAY-004 | お手本再生中は録音停止 |
| 録音排他 | REC-007 | 録音開始時はお手本停止 |
| 連続練習 | REC-008 | テイク自動分割フロー |
| ピッチ可視化 | PITCH-007 | ピッチ差分グラフ |
| 削除 | DATA-006 | Takeの個別削除（undo） |
| 削除 | DATA-007 | フレーズ詳細からのTake削除 |
