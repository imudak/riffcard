...
ユーザーがループボタンをタップしたとき、システムはお手本音声を繰り返し再生し続ける。もう一度タップすると停止する。

#### REQ-RC-PLAY-006: お手本再生速度調整

**種別**: Event-Driven

ユーザーが速度セレクター（0.5x / 0.75x / 1.0x）を選択したとき、システムは選択した速度でお手本音声を再生する。

#### REQ-RC-UX-006: お手本録音中リアルタイムピッチ表示

**種別**: State-Driven

お手本録音中、システムはマイク入力のピッチをリアルタイムで音名（例: C4, D#5）として画面に表示する。

#### REQ-RC-UX-007: お手本音声波形表示

**種別**: State-Driven

お手本音声が存在するとき、システムはフレーズ詳細画面にお手本音声の波形を表示する。

---

### Sprint 2: 整えるボタン（ピッチ補正補助）

#### REQ-RC-PITCH-008: ピッチオフセット自動検出

**種別**: Event-Driven

スコア結果画面でユーザーが「整える」ボタンをタップしたとき、システムはお手本と録音のDTWアライメント結果から中央値ピッチ差（セント）を計算し、オフセット値として保持する。

#### REQ-RC-PITCH-009: オフセット適用後スコア再計算

**種別**: Event-Driven

ピッチオフセットが設定されているとき、システムはオフセット補正後のピッチ差で pitchScore を再計算し、「整え済みスコア」として並べて表示する。整えたスコアは参考値として表示し、Take には保存しない。

#### REQ-RC-PITCH-010: オフセット表示

**種別**: State-Driven

「整える」実行後、システムはオフセット量（例: "+2.3半音 高め"）をユーザーに分かりやすく表示する。

---

### Sprint 3: MIDI 正規化

#### REQ-RC-PITCH-011: MIDI量子化比較モード

**種別**: Event-Driven

ユーザーが「MIDI比較」モードを選択したとき、システムはピッチ曲線を最近傍MIDIノート番号に量子化（四捨五入）してから DTW アライメントを行い、ノート一致率（%）を計算する。

#### REQ-RC-PITCH-012: MIDIノート一致率表示

**種別**: State-Driven

MIDI比較モードの結果画面において、システムはピッチスコアの内訳として「MIDIノート一致率 XX%」を表示し、ピッチ差分グラフのY軸単位をセントからMIDIノート差に切り替える。

#### REQ-RC-PITCH-013: MIDI正規化後ピッチ差分グラフ

**種別**: State-Driven

MIDI比較モードの差分グラフにおいて、システムは量子化後のお手本ノートと録音ノートの差（整数値）を棒グラフで表示する。Y軸は -3〜+3 ノート範囲とし、一致しているノートは緑、ずれているノートは赤で色分けする。
...
| Sprint | 要件ID | 内容 |
|--------|--------|------|
| Sprint 1 | PLAY-005 | お手本ループ再生 |
| Sprint 1 | PLAY-006 | お手本再生速度調整（0.5x/0.75x/1.0x） |
| Sprint 1 | UX-006 | 録音中リアルタイムピッチ表示（音名） |
| Sprint 1 | UX-007 | お手本音声波形表示 |
| Sprint 2 | PITCH-008 | ピッチオフセット自動検出（整えるボタン） |
| Sprint 2 | PITCH-009 | オフセット適用後スコア再計算 |
| Sprint 2 | PITCH-010 | オフセット量表示 |
| Sprint 3 | PITCH-011 | MIDI量子化比較モード |
| Sprint 3 | PITCH-012 | MIDIノート一致率表示 |
| Sprint 3 | PITCH-013 | MIDI正規化後ピッチ差分グラフ |
